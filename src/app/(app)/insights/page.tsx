...
export default function InsightsPage() {
  ...
  useEffect(() => {
    document.body.classList.add('bg-insights');
    return () => document.body.classList.remove('bg-insights');
  }, []);
  ...
  const maxMonth = useMemo(() => {
    if (!summary) return 0;
    return Math.max(...summary.monthlyHistory.map((m) => m.total), 1);
  }, [summary]);

  if (!summary) {
    return (
      <div className="text-center text-on-surface-variant py-20">
        <Icon name="sync" className="animate-spin inline-block mr-2" /> Carregando insights...
      </div>
    );
  }
